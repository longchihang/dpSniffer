...
; Global $g_sWPCAP_DLL = "wpcap.dll"
; Global $g_sPACKET_DLL = "packet.dll"
; #include <MsgBoxConstants.au3>
Global $g_sWPCAP_DLL = @ScriptDir & "\" & @OSArch & "\" & OSPath() & "\wpcap.dll"
Global $g_sPACKET_DLL = @ScriptDir & "\" & @OSArch & "\" & OSPath() & "\packet.dll"
; *********************** Initialisation functions **************************

Func _PcapSetup()	; return WinPCAP version as full text or -1 if winpcap is not installed, and opens dll
	; MsgBox($MB_SYSTEMMODAL, "$g_sWPCAP_DLL  $g_sPACKET_DLL", $g_sWPCAP_DLL & @CRLF & $g_sPACKET_DLL)
	If Not FileExists($g_sPACKET_DLL) Then return -1
	global $Packet_dll=DllOpen($g_sPACKET_DLL)
	; MsgBox($MB_SYSTEMMODAL, "$g_sWPCAP_DLL  $g_sPACKET_DLL", "ok packet.dll")
	; If Not FileExists(@SystemDir & "\wpcap.dll") Then return -1
	; global $Pcap_dll=DllOpen ( @SystemDir & "\wpcap.dll" )
	If Not FileExists($g_sWPCAP_DLL) Then return -1
	global $Pcap_dll=DllOpen($g_sWPCAP_DLL)
	; MsgBox($MB_SYSTEMMODAL, "$g_sWPCAP_DLL  $g_sPACKET_DLL", "ok wpcap.dll")

	; MsgBox($MB_SYSTEMMODAL, Pcap_dll, "wpcap.dll")
...
Func _PcapGetDeviceList() ; returns 2D array with pcap devices (name;desc;mac;ipv4_addr;ipv4_netmask;ipv4_broadaddr;ipv6_addr;ipv6_netmask;ipv6_broadaddr;flags) or -1 if error
	...
	while ($next<>0)
		...
		Local $handle=DllCall($Packet_dll, "ptr:cdecl", "PacketOpenAdapter", "ptr", DllStructGetPtr($snames))	
		If IsPtr($handle[0]) Then
			...
			Local $status=DllCall($Packet_dll, "byte:cdecl", "PacketRequest", "ptr", $handle[0],"byte",0,"ptr",DllStructGetPtr($packetoiddata))
			If $status[0] Then
			...
			$status=DllCall($Packet_dll, "byte:cdecl", "PacketGetNetType", "ptr", $handle[0], "ptr", DllStructGetPtr($nettype)) 
			If $status[0] Then
				$list[$i][5]=DllStructGetData($nettype,2)
			EndIf
			DllCall($Packet_dll, "none:cdecl", "PacketCloseAdapter", "ptr", $handle[0])
		EndIf
